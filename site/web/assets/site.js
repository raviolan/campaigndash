...
function positionSearchResults() {
  ...
  const searchContainer = searchBox.closest('.search');
  if (!searchContainer) return;
  const rect = searchContainer.getBoundingClientRect();
  results.style.position = 'fixed';
  results.style.top = (window.scrollY + rect.bottom + 8) + 'px';
  results.style.left = (window.scrollX + rect.left) + 'px';
...
function doSearch(q) {
  ...
  for (const it of INDEX) {
    const hit = isTag ? (it.tags || []).some(t => t.toLowerCase().includes(term)) : (it.title.toLowerCase().includes(term) || (it.headings || []).some(h => h.toLowerCase().includes(term)) || (it.content || '').toLowerCase().includes(term));
    if (hit) out.push(it);
    if (out.length > 20) break;
  }
  if (!out.length) { results.style.display = 'none'; results.innerHTML = ''; return }

  // Clear results and rebuild safely
  results.innerHTML = '';
  out.forEach(it => {
    const resultDiv = document.createElement('div');
    const link = document.createElement('a');
    const url = '/' + it.id.replace(/\\/g, '/').replace(/\.md$/i, '.html').split('/').map(encodeURIComponent).join('/');
    link.href = url;
    ...
    matchLocation.className = 'match-location';
    matchLocation.style.fontSize = '12px';
    matchLocation.style.color = 'var(--muted)';
    matchLocation.style.marginTop = '3px';

    if (!isTag && it.title.toLowerCase().includes(term)) {
      matchLocation.textContent = '📄 Found in title';
    } else if (!isTag && (it.headings || []).some(h => h.toLowerCase().includes(term))) {
      const matchingHeading = (it.headings || []).find(h => h.toLowerCase().includes(term));
      matchLocation.textContent = '📑 Found in heading: ' + matchingHeading;
    } else if (!isTag && (it.content || '').toLowerCase().includes(term)) {
      matchLocation.textContent = '📝 Found in content';
    } else if (isTag) {
      matchLocation.textContent = '🏷️ Tagged: ' + term;
    }
    resultDiv.appendChild(matchLocation);
    ...
    if (it.content && !isTag) {
      ...
      contentPreview.style.color = 'var(--muted)';
      contentPreview.style.lineHeight = '1.3';
      contentPreview.style.maxHeight = '2.6em';
      contentPreview.style.overflow = 'hidden';
      contentPreview.textContent = '"' + it.content + '"';
  ...
  // Position results dropdown and show
  positionSearchResults();
  results.style.display = 'block';
}
if (searchBox) {
  searchBox.addEventListener('input', () => doSearch(searchBox.value));
  searchBox.addEventListener('blur', () => { setTimeout(() => results.style.display = 'none', 200) });
  searchBox.addEventListener('focus', () => { if (searchBox.value.trim()) doSearch(searchBox.value) });
}
window.addEventListener('resize', () => { if (results.style.display === 'block') positionSearchResults(); });
