...
// Collapsible headings in content: h2..h4 become toggles that collapse following content until next heading
(function(){
  const root = document.querySelector('.entity-body') || document.querySelector('article');
  if(!root) return;
  const levels = ['H2','H3','H4'];
  const headings = [...root.querySelectorAll(levels.join(','))];
  headings.forEach(h=>{
    // Skip the top-level entity tab labels
    if(h.closest('.entity-tabs')) return;
    const container = document.createElement('div'); container.className='collapsible-section';
    const content = document.createElement('div'); content.className='collapsible-content';
    // Move following siblings until next same-or-higher heading level
    let el = h.nextSibling; const stopAt = (tag)=> levels.indexOf(tag) <= levels.indexOf(h.tagName);
    const toMove = [];
    while(el){
      if(el.nodeType===1 && levels.includes(el.tagName) && stopAt(el.tagName)) break;
      const next = el.nextSibling; toMove.push(el); el = next;
    }
    if(!toMove.length) return;
    h.classList.add('collapsible-toggle'); h.setAttribute('tabindex','0'); h.setAttribute('role','button'); h.setAttribute('aria-expanded','true');
    h.parentNode.insertBefore(container, h);
    container.appendChild(h);
    toMove.forEach(n=> content.appendChild(n));
    container.appendChild(content);
    function toggle(){ const open = h.getAttribute('aria-expanded')==='true'; h.setAttribute('aria-expanded', String(!open)); content.style.display = open? 'none':'block'; }
