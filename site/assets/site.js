...
(function () {
  ...
  function render() {
    const list = loadFav();
    favList.innerHTML = list.length ? list.map((f, i) => '<div class="fav-item"><a class="fav-link" href="' + hrefFor(f.id) + '">' + (f.title || f.id) + '</a><button class="fav-remove" data-remove="' + i + '" title="Remove">✕</button></div>').join('') : '<div class="meta">No favorites</div>';
